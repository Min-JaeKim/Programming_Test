...
* 제출
  ...
  for tc in range(1, t + 1):
      ...
      arr[n//2 -  1][n//2 - 1], arr[(n//2)][(n//2)], arr[(n//2)][(n//2) - 1], arr[(n//2) - 1][(n//2)] = 2, 2, 1, 1
      for i in range(m):
  ...
  > 오우 쓋ㅌ,,, 문제를 제대로 안읽으면 죽는 병이라도 걸린건가,, 진짜 문제를 제대로 읽은 적이 손에 꼽는 것 같다.
  >
  > 그리고 여기 사이트는 왜그런지 ㅡㅡ 시간측정을 지맘대로 한다. 실행 시간 순으로 검색해서 최고로 짧은 코드를 내가 돌리면 길게 나온다.
  >
  > 그래서 모범답안을 참고할 수가 없다 짜증!
  >
  > .
  >
  > 내가 처음에 헤맨 것은,, 처음 시작할 때 돌을 중앙에 미리 놓고 시작한다는 점이다. 그걸 해결하지 못해서 1시간 정도 헤맨 것 같다. 참 어이가 없다
  >
  > 계산 방법은,, 일단 2로 설명한다.  2 1 2 와 같이 1이 사이에 있을 경우 임시변수를 센다. 그리고 2를 마주할 경우 break해서 tmp 수만큼 2로 바꿔 준다. 왜 이렇게 풀어줬냐면 중간에 돌이 하나만 있을 때 바뀌는 게 아니라 여러 개 있을 때에도 바뀌기 때문이다 예를 들어 2 1 1 2 일 때 1 두 개가 모조리 바뀌어 버린다. 문제에 설명이 없다.. 
  >
  > 마지막으로 0이 없을 때에는 계산하면 안되는데 그부분을 살짝 혼동했다. 끝.
